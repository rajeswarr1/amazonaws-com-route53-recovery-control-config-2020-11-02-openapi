...
func CreateclusterHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/cluster", cfg.BaseURL)
...
func CreateCreateclusterTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_cluster",
		mcp.WithDescription("Create a new cluster. A cluster is a set of redundant Regional endpoints against which you can run API calls to update or get the state of one or more routing controls. Each cluster has a name, status, Amazon Resource Name (ARN), and an array of the five cluster endpoints (one for each supported Amazon Web Services Region) that you can use with API calls to the cluster data plane."),
		...
		mcp.WithString("ClusterName", mcp.Required(), mcp.Description("Input parameter: The name of the cluster.")),
		mcp.WithObject("Tags", mcp.Description("Input parameter: The tags associated with the cluster.")),
	)
