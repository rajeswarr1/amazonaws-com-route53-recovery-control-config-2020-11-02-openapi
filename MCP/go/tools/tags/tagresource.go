...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_ResourceArn",
		...
		mcp.WithString("ResourceArn", mcp.Required(), mcp.Description("The Amazon Resource Name (ARN) for the resource that's tagged.")),
		mcp.WithObject("Tags", mcp.Required(), mcp.Description("Input parameter: The tags associated with the resource.")),
	)
