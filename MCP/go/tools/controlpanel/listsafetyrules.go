...
func ListsafetyrulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/controlpanel/%s/safetyrules%s", cfg.BaseURL, ControlPanelArn, queryString)
		...
		var result models.ListSafetyRulesResponse
...
func CreateListsafetyrulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_controlpanel_ControlPanelArn_safetyrules",
		mcp.WithDescription("List the safety rules (the assertion rules and gating rules) that you've defined for the routing controls in a control panel."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListsafetyrulesHandler(cfg),
	}
}
